$(document).ready(function() {
  ...
  function createCardnModal(response) {
    ...
    for (var i = 0; i < response.length; i++) {
      ...
      // data-target has to match with id of modal (line 68)
  ...
  function getSupplies(category) {
    document.getElementById("main-container").innerHTML = "";
    if (!category) {
      $.get("/api/supplies", function(response) {
        createCardnModal(response);
      });
    } else {
      $.get("/api/supplies/" + category, function(response) {
        createCardnModal(response);
      });
    }
  }
    ...
    $.get(
      ...
      function(response) {
        document.getElementById("userCart").innerHTML =
          "Cart(" + response.length + ")";
      }
    );
  }
  $(".drop-category").on("click", function() {
    if ($(this).text() === "All") {
      getSupplies();
    } else {
      getSupplies(
